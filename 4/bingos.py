...
def run_simulation(simulation: Simulation):
    for seq in simulation.sequences:
        for board in simulation.boards:
            if seq not in board.num_position: continue
            ...
            if (board.marked_row[pos_row] == 5 or board.marked_column[pos_column] == 5) and (
                ...
                get_final_score(seq, board)
                print(simulation.boards.index(board), simulation.boards[simulation.boards.index(board)])
                return

def get_final_score(last_number: int, board: Board):
    unmarked_sum = 0
